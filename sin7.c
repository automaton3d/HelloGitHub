/*
 * sin7.c  –  3-D integer-only cellular automaton
 *
...
void render(SDL_Renderer* renderer) {
    ...
    /* --- auto-optimise k every 20 frames --- */
    if(tick - opt_tick >= 20) {
        opt_tick = tick;
    ...
    /* --- blue reference circle (pass 2, z==L/2 only) --- */
    {
        ...
        for(int y=0; y<L; y++) {
            ...
            if(rr > 0 && rr < RADIUS) {
                float ref = rsin2f((float)rr, kref);
    ...
    for(int r=0; r<RADIUS - 6; r++) {
        if(count[r] > 0) {
            ...
            double ref_val = rsin2f((float)r, kref);
